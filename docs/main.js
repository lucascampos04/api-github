(function () {
    ...
    async function getUser(user) {
        const [profileResponse, reposResponse] = await Promise.all([
            fetch(`${url}/${user}?client_id=${client_id}&client_secret=${client_secret}`),
            fetch(`${url}/${user}/repos?per_page=${count}&sort=${sort}&client_id=${client_id}&client_secret=${client_secret}`)
        ]);
